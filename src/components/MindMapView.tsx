...
export const MindMapView = ({ notes, onNoteUpdate, onNoteSelect, activeNote }: MindMapViewProps) => {
  ...
  useEffect(() => {
    ...
    notes.forEach(note => {
      note.linkedNotes.forEach(linkedNoteId => {
        ...
        if (notes.find(n => n.id === linkedNoteId)) {
          initialEdges.push({
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      if (params.source && params.target) {
        ...
        if (sourceNote && targetNote && !sourceNote.linkedNotes.includes(params.target)) {
          const updatedNote = {
            ...sourceNote,
